...
def get_result_map(b_size, y_img):
    ...
    result_map = np.zeros((b_size, 256, 512, 21))
    ...
    bridge=(y_img==15)
    tunnel= (y_img==16)
    background = np.logical_not(person + car + road+sidewalk+building+ wall+ fence+pole+trafficLight+trafficSign+vegetation+ground+sky+rider+truck+bus+motorcycle+bicycle+bridge+tunnel)
    # if person==1:
    #    print("person!\n")
    # for i in range(20):
    ...
    result_map[:, :, :, 19] = np.where(bridge, 1, 0)
    result_map[:, :, :, 20] = np.where(tunnel, 1, 0)
    return result_map
