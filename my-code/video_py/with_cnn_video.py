...
def CNN_predict(pic):
    input=pic
    t_start = cv2.getTickCount()
    result_deep = model_CNN.predict(input)
    t_total = (cv2.getTickCount() - t_start) / cv2.getTickFrequency() * 1000
    print("Predict current frame with_CNN total time: %.3f ms" % t_total)
    return result_deep
...
#fcn2:
model_shallow = with_cnn_shallow(input_shape=(256, 512, 3))
#deep layer：
model_deep= with_cnn_deep(input_shape=(256,512,19),num_classes=len(CLASSES))
#（1,1）layer -the end：
...
try:
    model_CNN.load_weights('../weight/CNN_model_weight.h5', by_name=True)
except:
...
while True:
    ...
    if frame==0:
        ...
        key_feature=[]
        #shallow predict
        result_shallow=shallow_predict(pic=input_data,flag=flag) # 1 256 512 19
        #deep predict
        result=deep_predict(pic=result_shallow,flag=flag)   # 1 256 512 19
        #mask
        imgmask=mask(result)
        cv2.imwrite('./key/'+'key_' + str(frame) + '_mask.png', imgmask)
    ...
    if frame!=0:
       if c>theta:
          ...
          flag=0
          #shallow layer:
          result_shallow=shallow_predict(pic=input_data,flag=flag)  # 1 64 128 19
          tt=time.time()
          # mid_feature = np.concatenate([key_feature[len(key_feature) - 1], result_shallow], axis=3)
          mid_feature = (key_feature[len(key_feature)-1]+result_shallow)/2
          print('concatenate time:\t %d ms' % ((time.time() - tt) * 1000))
          result = CNN_predict(mid_feature)
          #mask
          imgmask=mask(result)
          cv2.imwrite('./current/'+'current_' + str(frame) + '_mask.png', imgmask)
          t_cc=time.time()
