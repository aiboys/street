...
def data_generator_cnn_with_val(b_size):
    f = h5py.File('feature4.h5', 'r')
    deep_feature= f['deep_feature_4'][:]
    # f1 = np.load('deep_feature_3.npy')  # 256 256 512 19
    # deep_feature = np.squeeze(deep_feature)            # 256 256 512 19
    # f2 = np.load('shallow_feature_3.npy')  #  256 256 512 19
    # f2=np.load('flow_1.npy')         # 256 256 512 2 19
    shallow_feature=f['shallow_feature_4'][:]
    # shallow_feature = np.squeeze(shallow_feature)

    label=f['label_4'][:]
    # label = np.load('label_4.npy')  # 256 256 512
    ...
    while True:
        ...
        for i in range(d_size):
            ...
            x.append((x_imgs1[idx]+x_imgs2[idx])/2)
