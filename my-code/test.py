...
if model_name == "fcn":
  model = fcn_8s(input_shape=(256, 512, 3), num_classes=19, lr_init=3e-4, lr_decay=5e-4)
   ...
   model = resnet(input_shape= (256,512,3),num_classes=19, lr_init=3e-4, lr_decay=5e-4)
...
cv2.imwrite('./output_img/origin_img.png',x_img)
...
t_start = cv2.getTickCount()
pred = model.predict(x_img)                           #(1,256,521,21)                        # 2000ms
t_total = (cv2.getTickCount() - t_start) / cv2.getTickFrequency() * 1000
